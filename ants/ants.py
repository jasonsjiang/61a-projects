        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    ...
                    assert self.ant is None, 'Two ants in {}'.format(self)
                # END Problem 9
        else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is None:
            return False
        elif self.place.ant.blocks_path is True:
            return True
        return False
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place != hive and place.entrance != None:
            if len(place.bees) != 0 and self.min_range <= transit <= self.max_range:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        bee_list = [b for b in self.place.bees]
        for i in bee_list:
            i.reduce_armor(self.damage)
        # END Problem 7
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is more costly but watersafe!"""
    name = 'Scuba'
    food_cost = 6
    watersafe = True
    implemented = True
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        list_o_bees = list(self.place.bees)
        for i in list_o_bees:
            i.reduce_armor(self.damage)
...
# END Problem 13
    ...
    def __init__(self):
        # BEGIN Problem 13
        if QueenAnt.only_queen >= 1:
            self.imposter = True
        QueenAnt.only_queen += 1
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while location.exit:
                ...
                if location.ant:
                    double.append(location.ant)
            for i in range(len(double)):
                if double[i].container and double[i].ant:
                    double[i] = double[i].ant
            for ant in double:
                if ant not in self.double_damage:
                    ant.damage = ant.damage * 2
            self.double_damage += double
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        if self.imposter:
            self.place.remove_insect(self)
        elif self.armor <= 0 and not self.imposter:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    normal, status = bee.action, effect(bee.action)
    def ACTION(colony):
        nonlocal duration
        if duration > 0:
            status(colony)
