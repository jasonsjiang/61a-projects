...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(proc, rest.map(lambda o: scheme_eval(o, env)), env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Length mismatch.')
        else:
            while formals:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lamb = Pair(target.second, expressions.second)
        env.define(target.first, do_lambda_form(lamb, env))
        return target.first
        # END PROBLEM 10
    else:
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN
        # Check atoms like in original scheme_eval
        # Keep evaluating `result` until it's not Thunk
