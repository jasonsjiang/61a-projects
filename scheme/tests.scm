...
(define year (+ (- (* (cubed 9) 3) (* mistake 5) ) 1))
; expect year
year
; expect 2018
...
(define another_one (- (square (square (square 3))) 1))
another_one
; expect 6560
...
(and 4 '(1 . 2) another_one False (/ 0 0))
; expect #f

(let ((x 4) (f (lambda (a b) (+ a b)))) (f (* x 2) x))
; expect 12
...
(define get-it-to-one (lambda (a j) (f (* a (/ j 3)))))
; expect get-it-to-one

(get-it-to-one 2 3)
; expect 1

(define moo (mu (sorryimvegan) 'drinkthismilk))
; expect moo

(moo '(try this artisanal brie))
; expect drinkthismilk
