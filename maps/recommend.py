...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    close = [[find_closest(restaurant_location(restaurant), centroids), restaurant] for restaurant in restaurants]
    return group_by_first(close)
    # END Question 4
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(restaurant) for restaurant in cluster]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    s_xy = sum([x*y for x,y in zip([i - mean(xs) for i in xs], [i - mean(ys) for i in ys])])
    b = s_xy / s_xx
    a = mean(ys) - b * mean(xs)
...
def rate_all(user, restaurants, feature_fns):
    ...
    # BEGIN Question 9
    all_ratings = {}
    user_r_names = [restaurant_name(r) for r in reviewed]
    for r in restaurants:
        if restaurant_name(r) not in user_r_names:
            all_ratings[restaurant_name(r)] = predictor(r)
        elif restaurant_name(r) in user_r_names:
